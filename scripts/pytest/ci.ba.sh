...
# Find all test files recursively under the tests/ directory
find tests -type f -name "test_*.py" | while read -r file; do
    set -x

    # Get the directory path relative to tests/ (e.g., "auth/api")
    folder=$(dirname "${file#tests/}")
...
# Re-run all of them for coverage
pytest tests -m "${markers}" --cov-report=term-missing
